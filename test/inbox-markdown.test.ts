...
test("builds relative Markdown paths from the destination note", () => {
    ...
    assert.equal(
        relativeMarkdownPath("Journal/2026-08-01.md", "People/Andi.md"),
        "../People/Andi.md"
    );
    assert.equal(
        relativeMarkdownPath("Journal/Daily/2026-08-01.md", "Journal/People/Andi.md"),
        "../People/Andi.md"
    );
    assert.equal(
        relativeMarkdownPath("Journal/2026-08-01.md", "Journal/Andi.md"),
        "Andi.md"
    );
});
...
test("encodes non-ASCII path segments without encoding parent traversal", () => {
    assert.equal(
        relativeMarkdownPath("Journal/Harian.md", "People/Sutami café.md"),
        "../People/Sutami%20caf%C3%A9.md"
    );
});
