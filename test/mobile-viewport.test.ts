...
test("uses the window viewport when visualViewport is unavailable", () => {
    assert.deepEqual(getMobileViewportMetrics(640, undefined, 40, 8), { height: 592, offsetTop: 48, keyboardInset: 0 });
});

test("keeps a minimum system-bar clearance when the workspace starts at zero", () => {
    assert.deepEqual(getMobileViewportMetrics(844, { height: 844, offsetTop: 0 }, 0, 8), {
        height: 796,
        offsetTop: 48,
        keyboardInset: 0,
    });
