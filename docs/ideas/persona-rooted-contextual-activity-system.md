...
- [ ] Daily Notes users understand one mixed heading when Event and Task retain visibly different formats. Owner: Task 18 dogfooding and Checkpoint D.
- [ ] Opening the related Daily Note provides sufficient retrieval without linking to an exact bullet. Owner: Task 18 dogfooding and Checkpoint D.
- [ ] Self-contained append-only related logs remain useful after Project archival. Owner: Task 12 golden-fixture review and Task 18 dogfooding.
- [ ] Users can understand fleeting versus promoted modes without extensive onboarding. Owner: Task 18 dogfooding and Checkpoint D.
- [ ] Manually configured folders plus optional `type` filters cover real object layouts without excessive setup. Owner: Tasks 7, 8, and 18.
- [ ] A shared in-memory suggestion index remains responsive in representative desktop and mobile vaults. Owner: Task 8 fail-fast baseline and Task 17 real-vault confirmation.
- [ ] Keeping temporal metadata on a promoted Task's Daily Note link is understandable and avoids conflicting canonical values. Owner: Tasks 14, 16, and 18.
...
### Engineering prerequisites for MVP delivery

The following work is not additional product scope. It is the minimum reliability cost of extending the existing submission and related-note write path safely:

- typed submission outcomes with an explicit primary-write commit boundary;
- one shared in-flight submission policy across desktop and mobile;
- validation that prevents invalid temporal records from reaching the writer;
- recoverable, ordered settings reads and writes before migration;
- direct writer/parser compatibility tests before changing default headings;
- failed-destination-only recovery after partial contextual-log writes.
...
- What deterministic precedence applies when one note matches multiple enabled context sources through overlapping folder and property filters?
