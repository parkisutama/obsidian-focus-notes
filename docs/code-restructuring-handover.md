...
Tanggal: 2026-08-24
Status: Task 7 sampai Task 12 selesai. `EventTaskModal.ts`/`EventTaskMobileScreen.ts` sudah dipangkas jadi shell Moment-only, fungsi routing lintas-kind sudah diekstrak ke `EventTaskCaptureLauncher.ts` (lihat catatan Task 9 di bawah). Task 10: `SettingsTab.ts` didekomposisi dari 1355 baris jadi 169-baris thin navigation shell. Task 11: `TimerView.ts` didekomposisi dari 700 baris jadi 114-baris thin ItemView shell. Task 12: `TimelineView.ts` didekomposisi dari 442 baris jadi 196-baris thin ItemView shell — `TimelineIndex` (indexing/query, `refreshIndex()` mengembalikan status alih-alih langsung render), `TimelineHeader` (navigasi, callback-driven), `TimelineModalLauncher` (modal launching), dan `TimelineContentRenderer` (grid/sidebar body, meng-compose ketiganya) diekstrak ke `features/timeline/ui/`. Semua task ini pure code motion, tanpa perubahan behavior. **Manual acceptance testing desktop/mobile untuk Task 9 dilewati atas persetujuan eksplisit pengguna** (2026-08-24), bukan terlewat; Task 10/11/12 punya gap yang sama (tidak ada automated coverage untuk lapisan UI Obsidian-live), dicatat eksplisit di baseline doc.
Tujuan berikutnya: Task 13 — Thin plugin composition and remove shims, atau reprioritisasi sesuai arahan pengguna berikutnya.
...
8. Task 12 selesai (2026-08-24): `TimelineView.ts` didekomposisi empat batch (indexing/query orchestration, modal launching, navigation/header, grid/sidebar content rendering), turun dari 442 ke 196 baris. Tidak ada pekerjaan Task 12 lain yang terbuka. Mulai Task 13 (Thin plugin composition and remove shims) di sesi berikutnya kecuali prioritas berubah.
9. Pertahankan source import tetap acyclic; central `types.ts` dan seluruh re-export kontrak settings sudah dihapus setelah direct-consumer cutover selesai.
10. Pertahankan dependency advisory sebagai batch dependency-only yang terpisah dari restrukturisasi source.
