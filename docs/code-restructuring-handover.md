...
Tanggal: 2026-08-22
Status: Implementasi bertahap aktif; checkpoint Capture, Object Source, dan Timeline settings selesai tanpa perubahan perilaku
...
| `src/types.ts` | 433 | composition settings tersisa, persistence defaults, migration merge |
...
- `types.ts` menjadi titik dependensi bersama bagi beberapa domain yang sebenarnya berbeda.
...
- Cycle `types.ts`/Scheduled Item dan seluruh source cycle sudah diputus. Kontrak Capture, Object Source, serta Timeline settings kini canonical di feature domain masing-masing, sedangkan `InsertPosition` berada di `shared/markdown`; seluruh re-export kompatibilitas terkait sudah dihapus setelah zero-reference proof.
...
5. Lanjutkan Task 5 dengan satu owner per commit. Kandidat terdekat: Event/Task detail settings setelah consumer map dikonfirmasi; Capture, Object Source, dan Timeline settings sudah selesai.
6. Pertahankan source import tetap acyclic; shim `EventTaskWriter.ts` dan re-export central untuk Capture/Object Source sudah dihapus setelah direct-consumer cutover selesai.
