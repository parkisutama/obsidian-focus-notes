...
Tanggal: 2026-08-24
Status: Task 7, Task 8, Task 9, Task 10, dan Task 11 selesai. `EventTaskModal.ts`/`EventTaskMobileScreen.ts` sudah dipangkas jadi shell Moment-only, fungsi routing lintas-kind sudah diekstrak ke `EventTaskCaptureLauncher.ts` (lihat catatan Task 9 di bawah). Task 10: `SettingsTab.ts` didekomposisi dari 1355 baris jadi 169-baris thin navigation shell — lima kategori masing-masing sekarang punya satu file di `features/settings/ui/` menerima `SettingsRenderContext` alih-alih seluruh instance plugin. Task 11: `TimerView.ts` didekomposisi dari 700 baris jadi 114-baris thin ItemView shell — `TimerEngine.ts` (sudah murni sejak awal) dipindah ke `features/focus-session/domain/`; `CircularDisplay.ts`/`LogModal.ts` dipindah ke `features/focus-session/ui/`; empat concern baru (`TimerControls`, `TimerTargetEditor`, `TimerRecentEntries`, `TimerLogWorkflow`) diekstrak ke sana juga, masing-masing menerima app/settings/engine/callback sempit alih-alih seluruh ItemView. Tidak ada perubahan behavior — pure code motion untuk kedua task. **Manual acceptance testing desktop/mobile untuk Task 9 dilewati atas persetujuan eksplisit pengguna** (2026-08-24), bukan terlewat; Task 10/11 punya gap yang sama (tidak ada automated coverage untuk lapisan UI Obsidian-live), dicatat eksplisit di baseline doc.
Tujuan berikutnya: Task 12 — Decompose Timeline, atau reprioritisasi sesuai arahan pengguna berikutnya.
...
7. Task 11 selesai (2026-08-24): `TimerEngine.ts`/`CircularDisplay.ts`/`LogModal.ts` dipindah ke `features/focus-session/` (batch 1), lalu `TimerView.ts` didekomposisi empat batch (target editor, recent entries, completion/log orchestration, controls/presentation), turun dari 700 ke 114 baris. Tidak ada pekerjaan Task 11 lain yang terbuka. Mulai Task 12 (Decompose Timeline) di sesi berikutnya kecuali prioritas berubah.
8. Pertahankan source import tetap acyclic; central `types.ts` dan seluruh re-export kontrak settings sudah dihapus setelah direct-consumer cutover selesai.
9. Pertahankan dependency advisory sebagai batch dependency-only yang terpisah dari restrukturisasi source.
