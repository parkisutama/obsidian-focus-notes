...
Tanggal: 2026-08-22
Status: Rencana awal; belum ada pemindahan atau perubahan perilaku
...
| `src/types.ts` | 595 | settings, focus session, capture, object source, persistence defaults, migration merge |
...
- `ScheduledItemTypes.ts` masih mengimpor tipe dari `types.ts`, sementara `types.ts` mengimpor dan mengekspor ulang `TimelineMode`; ini menunjukkan batas tipe yang belum bersih.
...
6. Pertahankan source import tetap acyclic; kurangi shim tipe `EventTaskWriter.ts` melalui direct-consumer cutover dalam batch kecil.
