...
| Scheduled Item form semantics | `features/capture/scheduled-item/domain/ScheduledItemFormData.ts`, `features/capture/scheduled-item/domain/ScheduledItemFormAdapter.ts`, `EventTaskFormState.ts`, `SubmissionPolicy.ts` | Form data dan adapter/validation sudah canonical; seluruh consumer (desktop UI, mobile UI, application) direct-import dan shim root telah dihapus. `EventTaskFormState.ts`/`SubmissionPolicy.ts` masih di root menunggu keputusan owner |
...
1. `SubmissionPolicy.ts` adalah generic in-flight/completed guard untuk submission (bukan model Scheduled Item), pure, acyclic, dengan hanya 2 consumer produksi (`EventTaskModal.ts`, `EventTaskMobileScreen.ts`) dan 1 test file (`submission-policy.test.ts`) — kandidat kecil berikutnya jika owner-nya dikonfirmasi tetap Scheduled Item, bukan capture umum.
2. `EventTaskFormState.ts` masih mereferensikan `InboxRecord` milik Moment (state gabungan Moment/Event/Task); tentukan owner akhirnya (capture bersama vs Scheduled Item) sebelum memindahkannya — jangan pindahkan tanpa keputusan ini.
3. `ScheduledItemEditSubmission.ts` dan `ScheduledItemIdentityMigration.ts` adalah application/orchestration (bukan pure domain) menurut ownership map; itu milik Task 8, bukan Task 7.
4. Setelah butir 1–2 selesai atau diputuskan ditunda, verifikasi tiga acceptance criteria Task 7 (no Obsidian/DOM import di modul pure, satu kontrak semantik create/edit, parser legacy byte-identical) lalu tandai Task 7 pada `tasks/code-restructuring-todo.md` dan lanjut ke Task 8.

Checkpoint 2026-08-23 (lanjutan): `ScheduledItemFormAdapter.ts` sudah canonical di `features/capture/scheduled-item/domain/`. Seluruh 6 consumer produksi (`DetailNotePromotion.ts`, `ScheduledItemDesktopCreateModal.ts`, `ScheduledItemDesktopEditModal.ts`, `ScheduledItemEditSubmission.ts`, `ScheduledItemMobileCreateScreen.ts`, `ScheduledItemMobileEditScreen.ts`) dan 2 test file sudah cut over langsung, shim root telah dihapus, source tetap acyclic, dan full CI dengan 306 tes diverifikasi pada checkpoint ini. Dengan ini, parsing, block editing, validation, dan semantic adapters Scheduled Item sudah canonical di domain — hanya `EventTaskFormState.ts` dan `SubmissionPolicy.ts` yang tersisa di root menunggu keputusan owner.
