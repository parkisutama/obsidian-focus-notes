...
1. Verifikasi acceptance criteria kedua Task 8 (validasi Markdown/settings/path eksternal sebelum persist) pada adapter `infrastructure/obsidian/` yang baru dipindah; tambahkan abuse case untuk path traversal/malformed jika coverage belum ada.
...
3. `ContextLinkResolver.ts`, `RelatedWriteRecovery.ts`, `RelatedLog.ts`, `ObjectReference.ts` tetap di root — dipakai bersama oleh Scheduled Item application dan legacy `EventTaskSubmission.ts`; putuskan ownership akhirnya (shared `features/capture/application/`) secara terpisah, tidak memblokir Task 8.
4. `EventTaskFormState.ts`, `SubmissionPolicy.ts`, dan `EventTaskSubmission.ts` tetap di root sampai Task 9 (retirement jalur legacy `EventTaskModal.ts`/`EventTaskMobileScreen.ts`); ketiganya menggabungkan Moment dengan Event/Task melalui satu form/modal.
5. `Suggesters.ts` belum dipindah ke `infrastructure/obsidian/`; kandidat kecil berikutnya jika terbukti pure adapter serupa.

Checkpoint 2026-08-23 (Task 8 batch 2): `saveScheduledItemBlock` (dari `ScheduledItemBlockPersistence.ts`), `EventTaskWriter.ts`, `ObsidianLinkResolver.ts`, `ObsidianInboxSuggestionSource.ts`, dan `ObsidianScheduledItemMentionSource.ts` sudah canonical di `infrastructure/obsidian/`, mengikuti pola fungsi `async (app: App, ...)` yang sudah ada (`VaultFolders.ts`) — bukan interface port/DI baru. Seluruh 8 consumer produksi (termasuk legacy `EventTaskModal.ts`/`EventTaskMobileScreen.ts`) dan test file terkait sudah cut over langsung, shim root telah dihapus, source tetap acyclic, dan full CI dengan 306 tes diverifikasi.
