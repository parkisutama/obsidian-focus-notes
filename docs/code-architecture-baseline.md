...
1. Pindahkan adapter Obsidian yang menyentuh Vault langsung ke `infrastructure/obsidian/`: `saveScheduledItemBlock` (dari `ScheduledItemBlockPersistence.ts`), `EventTaskWriter.ts`, `ObsidianLinkResolver.ts`, `ObsidianInboxSuggestionSource.ts`, `ObsidianScheduledItemMentionSource.ts`. Pola yang sudah ada di codebase ini adalah fungsi `async (app: App, ...) => ...` biasa (lihat `infrastructure/obsidian/VaultFolders.ts`), bukan interface port/DI — ikuti pola itu, jangan perkenalkan abstraksi baru.
...
3. Setelah adapter Obsidian dipindah, verifikasi acceptance criteria kedua Task 8 (validasi Markdown/settings/path eksternal sebelum persist) pada boundary tersebut; tambahkan abuse case untuk path traversal/malformed jika coverage belum ada.
4. `ContextLinkResolver.ts`, `RelatedWriteRecovery.ts`, `RelatedLog.ts`, `ObjectReference.ts` tetap di root — dipakai bersama oleh Scheduled Item application dan legacy `EventTaskSubmission.ts`; putuskan ownership akhirnya (shared `features/capture/application/`) secara terpisah, tidak memblokir Task 8.
5. `EventTaskFormState.ts`, `SubmissionPolicy.ts`, dan `EventTaskSubmission.ts` tetap di root sampai Task 9 (retirement jalur legacy `EventTaskModal.ts`/`EventTaskMobileScreen.ts`); ketiganya menggabungkan Moment dengan Event/Task melalui satu form/modal.

Checkpoint 2026-08-23 (Task 8 batch 1): `ScheduledItemEditSubmission.ts` dan `ScheduledItemCreateRelated.ts` sudah canonical di `features/capture/scheduled-item/application/`. Keduanya sudah bebas Obsidian sebelum dipindah (dependency-injection based); ini murni perpindahan organisasi. Seluruh 4 consumer UI (desktop/mobile create/edit) dan test file terkait sudah cut over langsung, shim root telah dihapus, source tetap acyclic, dan full CI dengan 306 tes diverifikasi. Ditemukan bahwa `EventTaskSubmission.ts` sebenarnya legacy-scoped (hanya dipakai `EventTaskModal.ts`/`EventTaskMobileScreen.ts`/`SubmissionPolicy.ts`), bukan Scheduled Item application seperti dugaan awal peta ownership — dikoreksi di baris di atas.
