...
4. Rename `EventTaskModal.ts`/`EventTaskMobileScreen.ts` **dipertimbangkan dan ditolak** untuk sekarang: keduanya masih meng-export fungsi routing lintas-kind (`openEventTaskForm`, `openDesktopScheduledItemCreate`) yang bukan Moment-eksklusif. Rename tanpa memisahkan routing dari class render dulu berisiko salah label. Kalau mau dilanjutkan, langkah pertamanya adalah ekstraksi fungsi routing itu ke file terpisah (mis. `features/capture/domain` atau kandidat "Capture launch/orchestration" yang sudah ada), baru class render Moment-nya sendiri bisa direname/dipindah dengan aman.
5. Sisa file di root yang masih menyebut "EventTask" tapi sudah Moment-eksklusif: tidak ada lagi setelah `EventTaskSubmission.ts` dipindah — cek ulang jika ada penambahan baru.

Checkpoint 2026-08-24 (Task 9 batch 2): `EventTaskSubmission.ts` (`submitInbox`, `retryRelatedSubmission`, tipe hasil bersama — sudah Moment-eksklusif sejak `submitEventTask` dihapus) dipindah ke `features/capture/moment/application/EventTaskSubmission.ts`, nama file/simbol dipertahankan. Seluruh consumer (`EventTaskModal.ts`, `EventTaskMobileScreen.ts`, `SubmissionPolicy.ts`, 2 test file) direct-import, shim root telah dihapus, full CI dengan 299 tes diverifikasi. Rename `EventTaskModal.ts`/`EventTaskMobileScreen.ts` dievaluasi dan ditolak (lihat catatan ownership table). Manual acceptance testing dilewati atas persetujuan eksplisit pengguna.
