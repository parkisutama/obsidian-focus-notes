...
| Scheduled Item form semantics | `features/capture/scheduled-item/domain/ScheduledItemFormData.ts`, `ScheduledItemFormAdapter.ts`, `EventTaskFormState.ts`, `SubmissionPolicy.ts` | Form data contract sudah canonical; seluruh consumer (desktop UI, mobile UI, application) direct-import dan shim root telah dihapus. `ScheduledItemFormAdapter.ts` pure/acyclic dan menyusul berikutnya |
...
1. Pindahkan `ScheduledItemFormAdapter.ts` ke domain Scheduled Item dengan shim sementara; file ini sudah acyclic dan pure (`LedgerRecordSnapshot` dari `LedgerRecordSource.ts` hanya dipakai sebagai type).
2. Cut over 6 consumer produksi (`DetailNotePromotion.ts`, `ScheduledItemDesktopCreateModal.ts`, `ScheduledItemDesktopEditModal.ts`, `ScheduledItemEditSubmission.ts`, `ScheduledItemMobileCreateScreen.ts`, `ScheduledItemMobileEditScreen.ts`) dan 2 test file (`legacy-hub-ui-retirement.test.ts`, `scheduled-item-form-adapter.test.ts`) dalam batch kecil, lalu hapus shim setelah zero-reference proof.
3. `EventTaskFormState.ts` dan `SubmissionPolicy.ts` juga termasuk "Scheduled Item form semantics" pada peta ownership tapi `EventTaskFormState.ts` masih mereferensikan `InboxRecord` milik Moment; tentukan owner akhirnya (capture bersama vs Scheduled Item) sebelum memindahkannya.
4. `ScheduledItemEditSubmission.ts` dan `ScheduledItemIdentityMigration.ts` adalah application/orchestration (bukan pure domain) menurut ownership map; biarkan di root untuk Task 8, bukan Task 7.

Checkpoint 2026-08-23 (lanjutan): `ScheduledItemFormData.ts` sudah canonical di `features/capture/scheduled-item/domain/`. Seluruh consumer (desktop UI, mobile UI, application: `DetailNotePromotion.ts`, `ScheduledItemCreateRelated.ts`, `ScheduledItemEditSubmission.ts`, `ScheduledItemFormAdapter.ts`) dan test file terkait sudah cut over langsung per area, shim root telah dihapus, source tetap acyclic, dan full CI dengan 306 tes diverifikasi pada checkpoint ini.
