...
1. `EventTaskFormState.ts` tetap tidak bisa dipindah/disederhanakan — masih dipakai `ScheduledItemDesktopCreateModal.ts`/`ScheduledItemMobileCreateScreen.ts` sebagai default-value seed lewat `scheduledItemFormDataFromCreateState`. Field Event/Task di dalamnya (eventDate, taskPriority, dst.) masih diperlukan untuk nilai default itu, bukan sekadar warisan modal lama.
2. `SubmissionPolicy.ts` tetap dipakai `EventTaskModal.ts`/`EventTaskMobileScreen.ts` untuk submission Moment; tidak berubah.
3. `EventTaskSubmission.ts` sudah mengecil ke `submitInbox`/`retryRelatedSubmission`/tipe hasil bersama saja — evaluasi apakah rename/reorganisasi lebih lanjut diperlukan, atau biarkan karena sudah cukup kecil dan jelas cakupannya (Moment-only).
4. Manual acceptance testing (desktop dan mobile) untuk Moment capture dan redirect tab Event/Task **belum dilakukan** dalam sesi ini — direkomendasikan sebelum branch ini dianggap final, sesuai gerbang plan untuk legacy deletion.
5. Kandidat retirement fisik `EventTaskModal.ts`/`EventTaskMobileScreen.ts` (rename/relokasi filenya sendiri ke lokasi yang mencerminkan perannya sebagai Moment shell) belum dieksekusi — isinya sudah bersih tapi nama file masih menyiratkan Event/Task.

Checkpoint 2026-08-24 (Task 9 batch 1): Ditemukan dan dibuktikan lewat tracing bahwa `EventTaskModal.ts`/`EventTaskMobileScreen.ts` hanya pernah dikonstruksi dengan kind "inbox" — `openEventTaskForm`/tombol tab Event/Task selalu redirect ke `ScheduledItemDesktopCreateModal.ts`/`ScheduledItemMobileCreateScreen.ts` sebelum field Event/Task dirender/dibaca. Kode render/submit Event/Task yang provably dead dihapus dari kedua shell (887→418 baris desktop, 764→371 baris mobile), lalu `submitEventTask` (di `EventTaskSubmission.ts`) dihapus setelah terbukti zero caller. `EventTaskFormState.ts`/`SubmissionPolicy.ts` sengaja tidak disentuh — keduanya masih load-bearing. Tes yang mengetes `submitEventTask` langsung dihapus/disederhanakan; cakupan Inbox dan retry generik dipertahankan. Full CI dengan 299 tes diverifikasi setelah setiap commit.
