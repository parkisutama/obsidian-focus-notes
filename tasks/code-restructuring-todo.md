...
- [ ] Acceptance criterion 2 (external Markdown/settings/path input validated before persistence) has not yet had a dedicated verification pass across the moved adapters; abuse-case coverage for malformed/traversal paths should be checked next.
- [ ] `ScheduledItemIdentityMigration.ts` remains application/orchestration, still depending on the writer type rather than a domain/application contract; unmoved.
