...
- [ ] `Suggesters.ts` has not been evaluated for a similar `infrastructure/obsidian/` move.
- [ ] `EventLedgerEditor.ts`/`TaskLedgerEditor.ts` remain application-scoped (not pure domain); not evaluated for a move alongside `ScheduledItemBlockPersistence.ts`.
- [ ] Remaining Task 8 work is dominated by ownership decisions (shared capture primitives, `Suggesters.ts`) rather than further mechanical moves; both mechanical acceptance criteria (Obsidian-free policies, external path validation) are now satisfied.
