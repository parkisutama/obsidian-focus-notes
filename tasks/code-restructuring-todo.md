...
- [ ] Manual desktop and real-mobile acceptance testing (Moment capture end-to-end, Event/Task tab redirect) has **not** been performed in this session — recommended before this branch is considered final, per the plan's gate on legacy deletion.
- [ ] Not yet done: evaluating whether `EventTaskModal.ts`/`EventTaskMobileScreen.ts` should be renamed to reflect their now Moment-only role, and whether `EventTaskSubmission.ts` (now just `submitInbox`/`retryRelatedSubmission`) needs further reorganization.
